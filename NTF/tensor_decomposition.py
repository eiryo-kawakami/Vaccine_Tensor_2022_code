...
for na_mask in na_mask_3D:
    ...
    # na_mask.to_csv("na_mask_"+str(i)+".txt",sep="\t")
    # i += 1
...
# rank=10
# err_1=calc_denoising_loss(df_3D_trim,n_rep=50,rank_max=rank,method="MSE") 
...
# fig.savefig("CUH_SmartDR_20210727_denoising_rank_estim.pdf")

# rank=10
# err_2=calc_ordinal_loss(df_3D_trim,n_rep=50,rank_max=rank,method="MSE") 
# rank_list=list(range(1,rank+1))
...
==============================================================================
"""    
def decomposition(data,na_mask,rank,item_sort,colnames):
    df_array=[]
    for i in data:
        # d_array=mms.fit_transform(i)
        d_array = np.array(i)
    ...
    res=tsd.non_negative_parafac(data_3dim,rank=rank,n_iter_max=5000,mask=mask,init="random")
    time=pd.DataFrame(res[1][0])
    time.to_csv("CUH_SmartDR_20210727_mask_temporal_module.txt",sep="\t")
    column=pd.DataFrame(res[1][2])
    column.to_csv("CUH_SmartDR_20210727_mask_spatial_module.txt",sep="\t")
    ...
    for i in range(len(outer)):
        ...
        print(basis)
        basis.to_csv("CUH_SmartDR_20210727_mask_basis_"+str(i+1)+".txt",sep="\t")
        fig = plt.figure()
        plt.rcParams['font.family'] = 'Arial'
        plt.rcParams["font.size"] = 15
        sns.heatmap(basis,cmap="Reds")
        # plt.show()
        fig.savefig("CUH_SmartDR_20210727_mask_basis"+str(i+1)+".pdf")
...
def draw_heatmap(data,rank,metric,method="ward"): 
    ...
    df_cluster.to_csv("CUH_SmartDR_20210727_mask_patient_score_with_cluster.tsv",sep="\t")
    ...
    fig.savefig("CUH_SmartDR_20210727_mask_coef.pdf")
...
res=decomposition(df_3D_trim,rank=4,na_mask=na_mask_3D_trim,item_sort=items,colnames=days)
patient_score = res[0].copy()
patient_score.index = df_3D_trim[0].index
patient_score.to_csv("CUH_SmartDR_20210727_mask_patient_score.tsv",sep="\t")
